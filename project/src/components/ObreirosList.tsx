...
import { Users, User, Heart, Award, Clock, MapPin, CreditCard, Building2, Edit2, Trash2, Search, Filter, X, Download, BarChart3, TrendingUp } from 'lucide-react';

export function ObreirosList() {
  const { obreiros, deleteObreiro, getTotalObreiros, getObreirosByTipo, getObreirosBySetor, getObreirosByPagamento } = useObreiros();
  ...
  const filteredObreiros = useMemo(() => {
    return obreiros.filter(o => {
      // Search filter (nome, campo, setor)
      if (searchTerm) {
  ...
  const getTipoColor = (tipo: string) => {
    switch (tipo) {
      ...
      case 'missionaria': return 'bg-pink-100 text-pink-800';
      case 'evangelista': return 'bg-green-100 text-green-800';
      case 'jubilado': return 'bg-gray-100 text-gray-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const obreirosByTipo = getObreirosByTipo();
  const obreirosBySetor = getObreirosBySetor();
          ...
          <p className="text-blue-600">
            ...
            {filteredObreiros.length !== obreiros.length && (
              <span className="ml-2 text-sm">
                ({filteredObreiros.length} de {obreiros.length} registros)
              </span>
            )}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-blue-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              {filteredObreiros.length !== obreiros.length && (
                <p className="text-xs text-gray-500 mt-1">Filtrado: {filteredObreiros.length}</p>
              )}
            </div>
        ...
        {filteredObreiros.length === 0 ? (
            ...
            {obreiros.length === 0 ? (
              <>
                ...
                <p className="text-gray-500">
                  Comece cadastrando obreiros usando o botão "Cadastrar Obreiro".
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-gray-500 mb-4">
                  Não há obreiros que correspondam aos filtros aplicados.
                </p>
                <button
                  onClick={clearAllFilters}
          ...
          filteredObreiros.map((obreiro) => (
            <div key={obreiro.id} className="bg-white rounded-xl shadow-sm border border-blue-100 overflow-hidden">
              {/* Header */}
              <div className={`p-4 ${
                obreiro.tipo === 'pastor' ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
                obreiro.tipo === 'missionaria' ? 'bg-gradient-to-r from-pink-500 to-pink-600' :
                obreiro.tipo === 'evangelista' ? 'bg-gradient-to-r from-green-500 to-green-600' :
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isAdmin() && (
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => deleteObreiro(obreiro.id)}
                  ...
                  {obreiro.pagamento.tipo === 'dinheiro' ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-xs text-gray-600 space-y-1 ml-6">
                        ...
                        {obreiro.pagamento.banco?.contaPoupanca && (
                          <p><span className="font-medium">Poupança:</span> {obreiro.pagamento.banco.contaPoupanca}</p>
                        )}
                        {obreiro.pagamento.banco?.contaCorrente && (
                          <p><span className="font-medium">Corrente:</span> {obreiro.pagamento.banco.contaCorrente}</p>
                        )}
                      </div>
                    </div>
                  )}
