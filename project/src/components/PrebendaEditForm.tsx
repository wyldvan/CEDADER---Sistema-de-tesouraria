...
import { Prebenda, PASTORS, MONTHS, SECTORS } from '../types';
import { Edit2, X, DollarSign, FileText, AlertTriangle, CheckCircle, Info, Heart, HandHeart } from 'lucide-react';

interface PrebendaEditFormProps {
  isOpen: boolean;
  onClose: () => void;
  prebenda: Prebenda;
}

export function PrebendaEditForm({ isOpen, onClose, prebenda }: PrebendaEditFormProps) {
  const [documentNumber, setDocumentNumber] = useState(prebenda.documentNumber || '');
  const [isAuxilio, setIsAuxilio] = useState(prebenda.isAuxilio || false);
  const [isPrebenda, setIsPrebenda] = useState(prebenda.isPrebenda || false);
  const [pastor, setPastor] = useState(prebenda.pastor);
  const [amount, setAmount] = useState(prebenda.amount.toString());
  const [month, setMonth] = useState(prebenda.month);
  const [sector, setSector] = useState('');
  const [field, setField] = useState(prebenda.field || '');
  const [description, setDescription] = useState(prebenda.description);
  const [paymentMethod, setPaymentMethod] = useState<'pix' | 'cash' | 'transfer'>(prebenda.paymentMethod);
  const [documentError, setDocumentError] = useState('');
  const [documentValidation, setDocumentValidation] = useState<{ isValid: boolean, message: string }>({ isValid: true, message: '' });
  
  const { updatePrebenda, prebendas } = usePrebenda();
  ...
  // Verificar se o número do documento já existe (excluindo a prebenda atual)
  const checkDocumentNumber = (docNumber: string) => {
    ...
    // 1. Verificar duplicatas (excluindo a prebenda atual)
    const existingPrebenda = prebendas.find(p => 
      p.id !== prebenda.id && // Excluir a prebenda atual
      p.documentNumber && 
      p.documentNumber.toLowerCase() === normalizedDocNumber
    );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updatePrebenda(prebenda.id, {
      ...
      isAuxilio: prebenda.type === 'exit' ? isAuxilio : undefined,
      isPrebenda: prebenda.type === 'exit' ? isPrebenda : undefined,
    });
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-gray-900 flex items-center space-x-2">
              ...
              <span>
                Editar {prebenda.type === 'entry' ? 'Entrada' : 'Saída'} - Auxílio
              </span>
            </h3>
            <button
              onClick={onClose}
