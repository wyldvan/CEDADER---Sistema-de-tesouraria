...
interface PrebendaFormProps {
  ...
  onClose: () => void;
  type: 'entry' | 'exit';
  editPrebenda?: Prebenda; // Adicionado para o modo de edição
}
...
export function PrebendaForm({ isOpen, onClose, type, editPrebenda }: PrebendaFormProps) {
  ...
  const { addPrebenda, prebendas } = usePrebenda(); // Supondo que exista updatePrebenda
  ...
  useEffect(() => {
    if (isOpen) {
      if (editPrebenda) {
        setDocumentNumber(editPrebenda.documentNumber || '');
        setIsAuxilio(editPrebenda.isAuxilio || false);
        setIsPrebenda(editPrebenda.isPrebenda || false);
        setPastor(editPrebenda.pastor || '');
        setAmount(formatCurrency(editPrebenda.amount)); // Formata para exibição
        setMonth(editPrebenda.month || '');
        // Lógica para setor e campo pode precisar de ajuste
        setSector(editPrebenda.sector || '');
        setField(editPrebenda.field || '');
        setDescription(editPrebenda.description || '');
        setPaymentMethod(editPrebenda.paymentMethod || 'pix');
      } else {
  ...
  const checkDocumentNumber = (docNumber: string) => {
    ...
    const existingPrebenda = prebendas.find(p =>
      p.documentNumber && p.documentNumber.toLowerCase() === normalizedDocNumber && p.id !== editPrebenda?.id
    );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 2. Validação
    if (!pastor || amountNumber <= 0 || !month || !description) {
    ...
    if (editPrebenda) {
      // updatePrebenda(editPrebenda.id, prebendaData);
    } else {
      addPrebenda(prebendaData);
    }
  ...
  // O JSX permanece o mesmo, pois a lógica já está nos handlers e no submit.
  return (
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-gray-900 flex items-center space-x-2">
              ...
              <span>{editPrebenda ? 'Editar' : 'Nova'} {type === 'entry' ? 'Entrada' : 'Saída'} - Auxílio</span>
            </h3>
            ...
            <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center space-x-3 p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" checked={isAuxilio} onChange={(e) => setIsAuxilio(e.target.checked)} className="w-4 h-4 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500" />
                  <Heart className="w-4 h-4 text-pink-500" />
                  <span className="font-medium text-gray-900">Auxílio</span>
                </label>
                <label className="flex items-center space-x-3 p-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input type="checkbox" checked={isPrebenda} onChange={(e) => setIsPrebenda(e.target.checked)} className="w-4 h-4 text-emerald-600 border-gray-300 rounded focus:ring-emerald-500" />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={documentNumber} onChange={(e) => handleDocumentNumberChange(e.target.value)} className={`w-full pl-10 pr-10 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500 transition-colors ${documentError || !documentValidation.isValid ? 'border-red-300 focus:border-red-500 bg-red-50' : documentNumber && documentValidation.isValid && !documentError ? 'border-green-300 focus:border-green-500 bg-green-50' : 'border-gray-300 focus:border-emerald-500'}`} placeholder="Ex: 001, NF-123, REC-456..." />
              ...
              {documentNumber && !documentError && documentValidation.isValid && (
                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-green-700 font-medium">✓ {documentValidation.message || 'Número do documento válido'}</span>
                  </div>
                </div>
              )}
              {!documentNumber && (
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-blue-700">Configure faixas de documentos em Configurações → Documentos</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <select value={pastor} onChange={(e) => setPastor(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
            ...
            <div>
              ...
              <select value={month} onChange={(e) => setMonth(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" required>
            ...
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500" rows={3} placeholder="Descreva a prebenda..." required />
            </div>

            {sector && (
              <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                <h4 className="text-sm font-medium text-emerald-800 mb-2 flex items-center space-x-1"><DollarSign className="w-4 h-4" /><span>{sector}</span></h4>
                <div className="text-xs text-emerald-700">
                  <span className="font-medium">Campos disponíveis:</span>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {getFieldsForSector(sector).map((fieldName) => (<span key={fieldName} className={`px-2 py-1 rounded text-xs ${field === fieldName ? 'bg-emerald-200 text-emerald-900 font-medium' : 'bg-emerald-100 text-emerald-800'}`}>{fieldName}</span>))}
                  </div>
                </div>
              </div>
            )}
