...
export function useFinancialGoals() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('cedader_financial_goals');
    if (saved) {
  ...
  const saveGoals = (newGoals: FinancialGoal[]) => {
    setGoals(newGoals);
    localStorage.setItem('cedader_financial_goals', JSON.stringify(newGoals));
  };
  ...
  const getActualAmount = (field: string, year: number, month?: string) => {
    ...
    total += yearTransactions.reduce((sum, t) => sum + t.amount, 0);
    total += yearRegistrations.reduce((sum, r) => sum + r.amount, 0);
    total += yearPrebendas.reduce((sum, p) => sum + p.amount, 0);
  ...
  const getGoalsProgress = (year: number): GoalProgress[] => {
    ...
    goals.filter(g => g.year === year && g.isActive).forEach(goal => {
      ...
      const annualPercentage = (annualActual / goal.annualGoal) * 100;
  ...
  const getGoalsSummary = (year: number) => {
    ...
    return {
      ...
      totalGoalAmount: goals.filter(g => g.year === year && g.isActive)
        .reduce((sum, g) => sum + g.annualGoal, 0),
